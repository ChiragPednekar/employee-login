-- The "your assigned locations are inactive" guard must only apply when the
-- org has actually opted into per-employee site restriction; otherwise an
-- employee whose office was disabled would be blocked even though every other
-- approved site is open to them.
...
begin
  ...
  if site_restriction_enabled() and has_location_assignment(v_emp.id)
     and not exists (select 1 from allowed_locations(v_emp.id)) then
